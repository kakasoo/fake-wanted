...
## Outline
[`@kakasoo/PROJECT`](https://github.com/kakasoo/fake-wanted) provides SDK (Software Development Kit) for convenience.

For the client developers who are connecting to this backend server, [`@kakasoo/PROJECT`](https://github.com/kakasoo/fake-wanted) provides not API documents like the Swagger, but provides the API interaction library, one of the typical SDK (Software Development Kit) for the convenience.
...
```bash
npm install --save @kakasoo/fake-wanted-api
```
...
## Usage
Import the `@kakasoo/fake-wanted-api` and enjoy the auto-completion.
