...
export namespace EntityUtil {
  ...
  export const merge =
    ...
    async <Key extends bigint | number | string>(props: IMergeProps<Key>): Promise<void> => {
      // FIND TARGET MODEL AND PRIMARY KEY
      const model: Prisma.DMMF.Model | undefined = Prisma.dmmf.datamodel.models.find((model) => model.name === table);
      if (model === undefined) throw new Error(`Error on EntityUtil.unify(): table ${table} does not exist.`);
      const key: Prisma.DMMF.Field | undefined = model.fields.find((field) => field.isId === true);
      if (key === undefined)
        throw new Error(`Error on EntityUtil.unify(): table ${table} does not have single columned primary key.`);

      // LIST UP DEPENDENCIES
      const dependencies: Prisma.DMMF.Field[] = model.fields.filter(
        (field) =>
          field.kind === "object" && typia.is<Prisma.ModelName>(field.type) && !field.relationFromFields?.length,
      );
      for (const dep of dependencies) {
        // GET TARGET TABLE MODEL AND FOREIGN COLUMN
        const target: Prisma.DMMF.Model = Prisma.dmmf.datamodel.models.find((model) => model.name === dep.type)!;
        const relation: Prisma.DMMF.Field = target.fields.find((field) => field.relationName === dep.relationName)!;
        if (relation.relationFromFields?.length !== 1)
          throw new Error(`Error on EntityUtil.unify(): table ${getName(target)} has multiple columned foreign key.`);
        const foreign: Prisma.DMMF.Field = target.fields.find((f) => f.name === relation.relationFromFields![0])!;
  ...
  const _Merge_unique_children =
    ...
    async (current: { model: Prisma.DMMF.Model; foreign: Prisma.DMMF.Field; unique: readonly string[] }) => {
      // GET PRIMARY KEY AND OTHER UNIQUE COLUMNS
      const primary: Prisma.DMMF.Field = current.model.fields.find((column) => column.isId === true)!;
      const group: string[] = current.unique.filter((column) => column !== current.foreign.name);
      ...
      const dict: HashMap<any[], any[]> = new HashMap(
        ...
        (x, y) => ranges.equal(x, y, (a, b) => JSON.stringify(a) === JSON.stringify(b)),
      );
      const recordList: any[] = await (client as any)[current.model.name].findMany({
        where: {
      ...
      for (const it of dict) {
        const index: number = it.second.findIndex((rec) => rec[current.foreign.name] === parent.keep);
        if (index === -1) continue;
  ...
  const getName = (x: { dbName?: string | null; name: string }): string => x.dbName ?? x.name;
}
