...
export namespace BbsArticleCommentSnapshotProvider {
  export namespace json {
    export const transform = (
      input: Prisma.bbs_article_comment_snapshotsGetPayload<ReturnType<typeof select>>,
  ...
  export const store =
    (comment: IEntity) =>
    async (input: IBbsArticleComment.IUpdate): Promise<IBbsArticleComment.ISnapshot> => {
      const snapshot = await MyGlobal.prisma.bbs_article_comment_snapshots.create({
        data: {
          ...collect(input),
